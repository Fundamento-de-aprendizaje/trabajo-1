...
import numpy as snp
import pandas as pd
from funciones import calcularMetricas, find_s, aplicarHipotesis, calcularMatrizDeConfusion
from funciones import separarDatosEnConjuntos, entrenar_naive_bayes, predecir_naive_bayes,calcular_curva_roc
...
for persona in prueba:
    ...
    #print("PRUEBA", probabilidades["OTORGADO"])
   ...
   # y_scores.append(probabilidades["OTORGADO"] if "OTORGADO" in probabilidades else 0)
    y_scores.append(probabilidades["OTORGADO"])
...
# fpr, tpr, umbrales = calcular_curva_roc(y_true, y_scores)
# for i in range(len(umbrales)):
#    print(f"Umbral: {umbrales[i]:.4f} | FPR: {fpr[i]:.3f} | TPR: {tpr[i]:.3f}")

# Graficar la curva ROC
# Umbral que queremos destacar
# umbral_objetivo = 0.04
# #ignora3
# # Buscar el índice del primer umbral menor o igual a 0.5
# indice_umbral = next((i for i, u in enumerate(umbrales) if u <= umbral_objetivo), None)

# if indice_umbral is not None:
#     plt.plot(fpr[indice_umbral], tpr[indice_umbral], 'ro', label=f'Umbral = {umbral_objetivo:.2f}')
#     plt.annotate(f"({fpr[indice_umbral]:.2f}, {tpr[indice_umbral]:.2f})",
#                  (fpr[indice_umbral], tpr[indice_umbral]),
#                  textcoords="offset points", xytext=(10,10), ha='center', color='white')
# ##################################################
...
# plt.plot(fpr[indice_umbral], tpr[indice_umbral], 'ro', label=f'Umbral = {umbral_objetivo:.2f}')
# plt.plot(fpr, tpr, color='#e74c3c', lw=2, label=f'Curva ROC (AUC = {roc_auc:.2f})')
...
# # ⬇️ Marcar umbral específico (ej. 0.5)
# umbral_objetivo = 0
# indice_umbral = next((i for i, u in enumerate(umbrales) if u <= umbral_objetivo), None)
...
# ⬇️ Marcar múltiples umbrales
umbrales_objetivo = [0.01, 0.02, 0.005]
# umbrales_objetivo = [random.choice(umbrales), random.choice(umbrales), random.choice(umbrales)]
colores = ['cyan', 'red', 'yellow']

for umbral, color in zip(umbrales_objetivo, colores):
    indice_umbral = next((i for i, u in enumerate(umbrales) if u <= umbral), None)
    if indice_umbral is not None:
        plt.plot(fpr[indice_umbral], tpr[indice_umbral],'*' , color=color, label=f'Umbral = {umbral:.4f}')
        plt.annotate(f"({fpr[indice_umbral]:.2f}, {tpr[indice_umbral]:.2f})",
                     (fpr[indice_umbral], tpr[indice_umbral]),
                     textcoords="offset points", xytext=(10, 10), ha='center', color='white')
